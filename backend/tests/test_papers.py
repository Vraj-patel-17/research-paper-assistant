...
def test_get_papers_invalid_limit_zero(client):
...
def test_get_papers_invalid_limit_max(client):
...
def test_get_papers_empty_search(client):
    response = client.get("/papers?search=definitely_nonexistent_paper_xyz")
...
def test_get_papers_large_offset(client):
    response = client.get("/papers?offset=999999")
